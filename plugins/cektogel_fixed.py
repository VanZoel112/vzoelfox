...
async def cek_togel_batch_handler(event):
    ...
    try:
        ...
        # Show loading
        loading_msg = await event.reply(f"{get_emoji('main')} {convert_font('Scanning recent messages...', 'mono')}")
...
{get_emoji('main')} Try using regular `{prefix}cektogel` command
            """.strip()
        ...
        # Build batch result
        batch_text = f"""
{get_emoji('main')} {convert_font('BATCH CEK TOGEL RESULT', 'mono')}

╔══════════════════════════════════╗
   {get_emoji('adder1')} {convert_font('RECENT USERS SUMMARY', 'mono')} {get_emoji('adder1')}
╚══════════════════════════════════╝

{get_emoji('check')} Found {len(unique_users)} unique users:
...
"""
        ...
        for i, (user_id, user) in enumerate(unique_users.items(), 1):
            try:
                ...
                batch_text += f"""
{get_emoji('adder' + str(min(i, 6)))} {convert_font(f'#{i}', 'bold')} {convert_font(full_name[:20], 'mono')}{flag_display}
{get_emoji('check')} {convert_font('ID:', 'mono')} {user_id} | {convert_font('Username:', 'mono')} {username_display[:15]}

"""
            except Exception as e:
                batch_text += f"{get_emoji('adder3')} #{i} Error: {str(e)}\n\n"
        
        batch_text += f"""
╔══════════════════════════════════╗
   {get_emoji('adder2')} {convert_font('DETAILED CHECK', 'mono')} {get_emoji('adder2')}
╚══════════════════════════════════╝

{get_emoji('main')} Use `{prefix}cektogel <user_id>` for detailed info
{get_emoji('adder4')} Or reply to specific message + `{prefix}cektogel`

{get_emoji('adder5')} {convert_font('Scanned at:', 'bold')} {datetime.now().strftime('%H:%M:%S')}
        """.strip()
        
        await safe_send_with_entities(event, batch_text)
        
    except Exception as e:
        await safe_send_with_entities(event, f"{get_emoji('adder3')} {convert_font('Batch error:', 'bold')} {str(e)}")
...
async def quick_info_handler(event):
    ...
    try:
        ...
        try:
            ...
            await safe_send_with_entities(event, f"{get_emoji('adder3')} User not found: {str(e)}")
            return
        
        if not user:
            usage_text = f"""
{get_emoji('check')} {convert_font('QUICK INFO USAGE', 'mono')}
...
• `{prefix}quickinfo 123456789`
        ...
        quick_text = f"""
{get_emoji('adder4')} {convert_font('QUICK USER INFO', 'mono')}

{get_emoji('check')} {convert_font(full_name, 'mono')}{flag_display}
{get_emoji('adder2')} {convert_font('ID:', 'bold')} {user.id}
{get_emoji('adder1')} {convert_font('Username:', 'bold')} {username_display}
{get_emoji('main')} {convert_font('Status:', 'bold')} {format_status(getattr(user, 'status', None))}

{get_emoji('adder5')} Use `{prefix}cektogel` for detailed info
        """.strip()
        
        await safe_send_with_entities(event, quick_text)
        
    except Exception as e:
        await safe_send_with_entities(event, f"{get_emoji('adder3')} {convert_font('Quick info error:', 'bold')} {str(e)}")
...
async def clear_cache_handler(event):
    ...
    try:
        ...
        clear_text = f"""
{get_emoji('adder2')} {convert_font('CACHE CLEARED', 'mono')}

{get_emoji('check')} Cleared {cache_count} cached user entries
{get_emoji('main')} Fresh data will be fetched on next check
{get_emoji('adder1')} Memory usage optimized
        """.strip()
        
        await safe_send_with_entities(event, clear_text)
        
    except Exception as e:
        await safe_send_with_entities(event, f"{get_emoji('adder3')} {convert_font('Cache clear error:', 'bold')} {str(e)}")
