...
# Blacklist file path
BLACKLIST_FILE = "data/gcast_blacklist.json"
...
def is_valid_chat_id(chat_id_str):
    ...
    try:
        ...
        # Telegram chat IDs are typically negative numbers for groups/channels
        # or positive for users, but we mainly deal with groups/channels for gcast
        return True
    except ValueError:
        return False
...
• `{prefix}clearbl` - Clear all blacklist
...
@client.on(events.NewMessage(pattern=r'\.clearbl'))
...
{get_emoji('adder1')} No chats to clear
        ...
        # For safety, require confirmation by sending the command twice quickly
        # This is a simple implementation - you might want a more sophisticated confirmation
        confirmation_text = f"""
...
{get_emoji('main')} Or use `{prefix}listbl` to review first
        ...
        if event.text and 'confirm' in event.text.lower():
            # Clear the blacklist
            empty_blacklist = {}
...
{get_emoji('main')} Check permissions and try again
        ...
        await safe_send_with_entities(event, f
